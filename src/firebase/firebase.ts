      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
        ...
        <div style={{ width: 188, borderRight: "1px solid rgba(255,255,255,0.07)", padding: "20px 14px", flexShrink: 0, display: "flex", flexDirection: "column", gap: 20, background: "rgba(255,255,255,0.01)", overflowY: "auto" }}>
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", minHeight: 0 }}>
            <p style={{ margin: "0 0 10px", fontSize: "0.6rem", color: "rgba(255,255,255,0.25)", letterSpacing: "0.12em", textTransform: "uppercase" }}>Room Chat</p>
            <div style={{ flex: 1, overflowY: "auto", display: "flex", flexDirection: "column", gap: 8, marginBottom: 8, minHeight: 80, maxHeight: 200 }}>
              {chatMessages.map((msg, i) => (
                <div key={i} style={{ fontSize: "0.72rem", lineHeight: 1.4 }}>
                  <span style={{ color: `hsl(${(msg.from?.charCodeAt(0) * 47) % 360},65%,65%)`, fontWeight: 600 }}>{msg.from}: </span>
                  <span style={{ color: "rgba(255,255,255,0.6)" }}>{msg.text}</span>
                </div>
              ))}
              {chatMessages.length === 0 && <p style={{ fontSize: "0.68rem", color: "rgba(255,255,255,0.12)", fontStyle: "italic", margin: 0 }}>No messages yet</p>}
            ...
            <div style={{ display: "flex", gap: 5 }}>
              <input value={chatInput} onChange={e => setChatInput(e.target.value)}
                ...
                placeholder="Message…"
                style={{ flex: 1, background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 8, color: "white", padding: "6px 8px", fontSize: "0.72rem", outline: "none" }} />
              <button onClick={sendChat} style={{ background: "linear-gradient(135deg,#f97b5c,#f4a26a)", border: "none", borderRadius: 8, color: "white", cursor: "pointer", padding: "6px 9px", fontSize: "0.75rem" }}>→</button>
            </div>
          </div>
        </div>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
          ...
          {/* Tab content */}
          <div style={{ flex: 1, overflow: tab === "syncnotes" ? "hidden" : "auto", padding: tab === "syncnotes" ? 0 : "32px 24px", display: "flex", flexDirection: "column" }}>
            ...
            {/* CANVAS — real-time via Whiteboard component */}
            {tab === "syncnotes" && (
                  ...
                  {libFiles.map(f => {
                    ...
                    return (
                      <div key={f.id} style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 13, padding: "13px 16px", display: "flex", alignItems: "center", gap: 12, animation: "fadeUp 0.3s ease" }}>
                        ...
                        <button onClick={() => deleteDoc(doc(db, "studyrooms", activeRoom.id, "library", f.id))} style={{ background: "none", border: "none", color: "rgba(255,100,80,0.35)", cursor: "pointer", fontSize: "0.85rem", padding: 0 }}>✕</button>
                      </div>
                    );
                  })}
                </div>
